...
drop procedure if exists ls_isis_spf;
DELIMITER //
CREATE PROCEDURE ls_isis_spf (IN peer_hash_id char(32),
                              IN root_router_id varchar(46),
...
BEGIN
    ...
    set igp_rib_table_name = concat(igp_rib_table_name,'_', input_mt_id,'_', replace(replace(root_router_id, '.', '_'),':', '_'));
    ...
    if (@table_age > max_table_age) THEN
        ...
        # temp table for the computed spf paths (links) for root node
        ...
        # Create IGP_RIB best/working table
        ...
        OPEN path_cursor;
        WHILE NOT done DO
            FETCH path_cursor INTO node_hash_id,nh_node_hash_id,node_router_id,path_node_hash_id,node_metric,node_spf_iter;
            ...
            ELSE
                ...
                IF (node_hash_id = nh_node_hash_id) THEN
                    # ROOT NODE
                    UPDATE spf_path p SET p.path_hash_ids = path_node_hash_id,
        ...
        set @sql_text = concat("
            INSERT IGNORE INTO ", igp_rib_table_name, " (prefix,prefix_len,src_node_hash_id,
                        isis_type,metric,best,path_hash_ids,path_router_ids,
                        equal_iter,nh_node_hash_id,root_node_hash_id,peer_hash_id,mt_id)
                SELECT
                        t.prefix,t.prefix_len,t.src_node_hash_id,t.isis_type,t.metric,
